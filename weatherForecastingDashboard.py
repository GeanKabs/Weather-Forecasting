...
st.set_page_config(layout="wide", initial_sidebar_state="expanded", page_title='Weather Forecasting Dashboard')
...
st.header('Mean Temperature in Delhi over the years')
...
# Humidity
st.header('Humidity in Delhi over the years')
figure = px.line(data, x="date",
		y="humidity",
...
# Wind Speed
st.header('Wind Speed in Delhi over the years')
figure = px.line(data, x="date",
		y="wind_speed",
...
selected_points = plotly_events(figure)
st.write('There is a negative correlation between temperature and humidity in Delhi. It means higher temperature results in low humidity and lower temperature results in high humidity.')
...
st.header('Temperature change in Delhi over the years')
plt.style.use('fivethirtyeight')
plt.figure(figsize=(15, 10))
plt.title("Temperature Change in Delhi Over the Years")
sns.lineplot(data = data, x='month', y='meantemp', hue='year')
st.pyplot()
st.write('Although 2017 was not the hottest year in the summer, we can see a rise in the average temperature of Delhi every year')

# Forecasting Weather using Python
st.header('Forecasting Weather')
st.write('The Facebook prophet model will be used for this task')

st.subheader('Converting the data format')
st.write('The prophet model accepts data named as "ds", and labels as "y" ')
